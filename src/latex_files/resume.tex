...
\begin{document}
    ...
    \begin{onecolentry}
      \begin{highlights}
        \item Build AWS-based ETL pipelines (Lambda/Glue) that extract, transform, and distribute fund, benchmark, and portfolio data from internal data lakes, APIs, and third-party sources to PostgreSQL and downstream applications.
        \item Develop the team's first React application for municipal bond analysis, including API Gateway integration and core data table workflows for fixed income stakeholders.
        \item Design Tableau hedged share class reporting and Plotly monitoring views to improve transparency across daily business-critical processes and reduce manual reporting overhead.
        \item Lead reliability and modernization work by upgrading PostgreSQL and AWS Glue components and migrating cron-based jobs to timezone-aware scheduling.
        \item Serve as a Security Champion by identifying vulnerabilities and partnering with engineers to prioritize and remediate application risks.
      \end{highlights}
    \end{onecolentry}
    \vspace{0.12 cm}
    \begin{twocolentry}{
    ...
    \begin{twocolentry}{
        ...
        \begin{onecolentry}
            \begin{highlights}
                \item Maintain a Python-based Discord bot with slash-command admin automation, including conditional roles, channel restrictions, message mirroring, and scheduled role changes backed by PostgreSQL.
            \end{highlights}
        \end{onecolentry}
        \vspace{0.12 cm}
        \begin{twocolentry}{
    ...
    \end{onecolentry}
    \begin{onecolentry}
      \textbf{Tools:} Git, AWS (Glue, Lambda, IAM, S3, Cloudformation, API Gateway, ECS), Docker
    \end{onecolentry}
\end{document}